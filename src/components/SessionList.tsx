...
const SessionList = ({ allSessions, fromPatientID, isShowDate = false }: any) => {
    return (
        <IonList>
            {allSessions && allSessions.map((session: any) => (
                <IonItemSliding key={session["🔒 Row ID"]}>
                    <IonItem button={true} key={session["🔒 Row ID"]} detail={true} href={`/viewsession/${session["🔒 Row ID"]}${fromPatientID ? `?fromPatientID=${fromPatientID}`:``}`}>
